...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='9d577d8a29893062dfbd669997396dbd01ab0e47'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD -> master, refs/__gh__/pull/108428/rebase, refs/__gh__/pull/108260/rebase' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD -> master, refs/__gh__/pull/108428/rebase, refs/__gh__/pull/108260/rebase' =~ tag:\ (v[^ ,]+) ]]; then
