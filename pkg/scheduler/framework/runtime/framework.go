...
// frameworkImpl是负责初始化和运行调度程序插件的组件
// 每一个frameworkImpl结构体对象，就是为某个Pod指定的专属调度器
// 其中指定了每个拓展点中的具体插件
type frameworkImpl struct {
	...
	// 过滤流程中的扩展点所对应的具体插件
	// 都是framework接口中的扩展点的切片类型，即一个扩展点中可添加多个符合接口方法格式的插件
...
// 在给定节点上为pod运行一组配置的过滤器插件。如果这些插件中的任何一个没有返回“成功”，则给定节点不适合运行pod。
// 同时，为给定节点设置失败消息和状态。
...
//  @Description:
//  	运行单个过滤器插件
//  @receiver f *frameworkImpl 对象
//  @param ctx 请求上下文
//  @param pl 过滤器插件
//  @param state Pod在整个调度流程中的状态信息
//  @param pod 待调度pod
//  @param nodeInfo 宿主机节点信息
//  @return *framework.Status 返回调度结果
//
...
// 在给定的节点上为指定的pod运行一组配置的过滤器插件。
// 这个函数从两个不同的地方调用：调度和抢占。
// 当它从调度调用时，我们希望测试pod是否在节点上是可调度的，节点上所有现有的pod加上指定在节点上运行的更高和相等优先级的pod。
// 当它从抢占中调用时，我们应该删除抢占的受害者并添加指定的pod。受害者的删除由SelectVictimsOnNode（）完成。
// 抢占在调用此函数之前从PreFilter状态和NodeInfo中删除受害者。

//
// RunFilterPluginsWithNominatedPods
//  @Description:
//  	在给定的宿主机节点上，为指定的Pod运行一组与其对应的过滤器插件
//  @receiver f *frameworkImpl 对象
//  @param ctx 请求上下文
//  @param state 储存调度Pod在整个调度流程中的状态数据
//  @param pod 待调度Pod
//  @param info 宿主机节点的信息
//  @return *framework.Status 返回过滤结果
//
func (f *frameworkImpl) RunFilterPluginsWithNominatedPods(ctx context.Context, state *framework.CycleState, pod *v1.Pod, info *framework.NodeInfo) *framework.Status {
	...
	// 在某些情况下，我们会运行两次过滤器。如果节点具有更大或相等优先级的指定pod，我们会在这些pod添加到PreFilter状态和nodeInfo时运行它们。
	// 如果所有过滤器在此次传递中成功，当这些指定的pod未添加时，我们会再次运行它们。这第二次传递是必要的，因为某些过滤器（例如pod间亲和力）可能在没有指定的pod的情况下无法通过。
	// 如果节点没有指定的pod，或者如果过滤器的第一次运行失败，我们不会运行第二次传递。
	// 我们在第一遍中只考虑相等或更高优先级的pod，因为这些是当前的“pod”必须屈服于它们，而不是占用为运行它们而打开的空间。如果当前的“pod”占用为低优先级pod释放的资源是可以的。
	// 要求新pod在这两种情况下都是可调度的，可以确保我们做出保守的决定：资源和pod之间的过滤器
	// 当处理指定的pod时，反亲和力更有可能失败
	// 在运行时，而pod亲和力等过滤器更有可能在以下情况下失败
	// 指定的pod被视为未运行。我们不能只是假设
	// 指定的pod正在运行，因为它们现在没有运行，事实上，
	// 它们最终可能会被安排到不同的节点。

	// 如果待检查的Node是一个即将被抢占的节点，调度器就会对这个Node用同样的Predicates算法运行两遍
	for i := 0; i < 2; i++ {
		stateToUse := state
		nodeInfoToUse := info
		// 处理优先级pod的逻辑
...
// RunScorePlugins运行一组配置的评分插件。它返回一个列表，该列表为每个评分插件名称存储相应的NodeScoreList。
// 它还返回*status，如果任何插件返回非成功状态，则将其设置为非成功。

//
// RunScorePlugins
//  @Description:
//  	运行score扩展点的插件,分别调用parallelize.Until方法跑三次来进行打分
//  @receiver f *frameworkImpl 对象
//  @param ctx 请求上下文
//  @param state 整个调度流程中该Pod的状态信息
//  @param pod 待调度的Pod
//  @param nodes 宿主机节点
//  @return ps 返回宿主机节点与其评分相映射的列表
//  @return status score插件的运行结果
//
func (f *frameworkImpl) RunScorePlugins(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodes []*v1.Node) (ps framework.PluginToNodeScores, status *framework.Status) {
	...
	// 并行运行每个节点的Score方法。
	// 1.第一次:调用runScorePlugin方法，里面会调用getDefaultConfig里面设置的score的Plugin来进行打分；
	f.Parallelizer().Until(ctx, len(nodes), func(index int) {
		// 遍历该Pod指定的score插件数组，并逐个运行
		for _, pl := range f.scorePlugins {
	...
	// 并行运行每个节点的额外score方法
	// 2.第二次调用runScoreExtension方法，里面会调用Plugin的NormalizeScore方法，用来保证分数必须是0到100之间，不是每一个plugin都会实现NormalizeScore方法。
	...
	// 根据插件的优先级对分数进行调整
	// 3.第三次会调用遍历所有的scorePlugins，并对对应的算出的来的分数乘以一个权重。
	f.Parallelizer().Until(ctx, len(f.scorePlugins), func(index int) {
		pl := f.scorePlugins[index]
...
// 运行一组配置的许可插件。如果这些插件中的任何一个返回“成功”或“等待”以外的状态，它不会继续运行剩余的插件并返回错误。否则，如果任何插件返回“等待”，则此函数将创建等待pod并将其添加到当前等待pod的映射中，并使用“等待”代码返回状态。
// Pod将在允许插件返回的最短持续时间内保持等待pod。
