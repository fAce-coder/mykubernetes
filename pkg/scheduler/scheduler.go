...
// 监视新的待调度的pod，尝试找到适合它们的节点，并将Pod和宿主机的绑定关系写回api服务器。
type Scheduler struct {
	// It is expected that changes made via Cache will be observed
	// by NodeLister and Algorithm.
	// NodeLister和调度算法，将感应到通过缓存 Cache 所做的更改。
	...
	// NextPod 下一个需要调度的Pod，应该是一个阻塞直到下一个pod可用的函数
	// 我们不为此使用通道，因为调度pod可能需要一些时间，并且我们不希望pod在通道中时变得陈旧
	NextPod func() *framework.QueuedPodInfo

	// Error is called if there is an error. It is passed the pod in
	// question, and the error
	// 如果有错误，则调用Error。它被传递给有问题和异常的pod
	Error func(*framework.QueuedPodInfo, error)

	// SchedulePod tries to schedule the given pod to one of the nodes in the node list.
	// Return a struct of ScheduleResult with the name of suggested host on success,
	// otherwise will return a FitError with reasons.
	// SchedulePod 尝试将给定的pod调度到宿主机节点列表中的一个节点，调度器的具体实现函数
	// 成功时返回带有建议宿主机名称的scheduleResult结构，否则将返回带有原因的FitError
	...
	// 调度器的配置文件，通过配置文件来加载某个Pod在每个拓展点要用哪些特定的插件
	// 存储着 调度器名(字符串string类型)：具体的调度器frameworkImpl对象(framework.Framework接口类型) 的多组映射关系
	...
	// 下次调度流程开始的宿主机索引标志位
	// 目的是记录上次调度参与的宿主机节点，下次从这个节点开始遍历，这样在集群规模很大时，能让每个宿主机都能公平的得到遍历
...
// 初始化 scheduler 结构体，该方法主要的功能是初始化默认的调度算法以及默认的调度器
...
func (sched *Scheduler) Run(ctx context.Context) {
	// 1.会将backoffQ中的Pods节点和unschedulableQ中的节点移至activeQ中。
	// 即：将之前运行失败的节点和已经等待了很长时间超过时间设定的节点重新进入活跃节点队列中。
	// backoffQ 是并发编程中常见的一种机制，就是如果一个任务重复执行，但依旧失败，则会按照失败的次数提高重试等待时间，避免频繁重试浪费资源。
	// unschedulableQ 是确定无法调度的
	...
	// 2.在一个专用的goroutine中启动SchedulOne循环，因为SchedulingQueue函数依赖于从SchedulingQueue中获取下一个项目。
	// 如果没有新的pod要调度，它将挂在那里，如果在这个goroutine中完成，它将阻塞关闭SchedulingQueue，实际上导致关闭时死锁。
	go wait.UntilWithContext(ctx, sched.scheduleOne, 0)

	<-ctx.Done()
	// 3.关闭调度之后，对队列也进行关闭。SchedulingQueue是一个优先队列
	// 优先作为实现SchedulingQueue的实现，其核心数据结构主要包含三个队列:activeQ、podBackoffQ、unschedulableQ
	// 内部通过cond来实现Pop操作的阻塞与通知：
	// 		当前队列中没有可调度的pod的时候，则通过cond.Wait来进行阻塞
	// 		然后在往activeQ中添加pod的时候通过cond.Broadcast来实现通知。
